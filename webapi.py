...
def init_timer(client_id):
    ...
    timers[client_id] = RepeatedTimer(10, save_session, client_id)


def save_session(client_id):
    ...
    try:
        ...
        res = drivers[client_id].save_firefox_profile(remove_old=False)
    except:
        pass
    finally:
        release_semaphore(client_id)
...
def before_request():
    ...
    auth_key = request.args.get('auth-key')
...
def get_qr():
    img_title = 'qr_' + g.client_id + '.png'
    image_path = STATIC_FILES_PATH + img_title
    if g.driver_status != WhatsAPIDriverStatus.LoggedIn:
        try:
            with open(image_path, "wb") as f:
                f.write(decodebytes(g.driver.get_qr_base64().encode()))
            return send_file(image_path, mimetype='image/png')
...
@app.route('/screen/qr_plain', methods=['GET'])
def get_qr_plain():
    """Get qr as a json string"""
    if "Click to reload QR code" in g.driver.driver.page_source:
        g.driver.reload_qr()
...
def get_unread_messages():
    ...
    unread_msg = g.driver.get_unread(use_unread_count=True)
